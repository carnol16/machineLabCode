...
while (true)
{
    ...
    if (GWindow.mouseLeftDown())
    {
        ...
        sphere.sca(.1);
        // position the sphere
        sphere.pos(GG.camera().screenCoordToWorldPos(GWindow.mousePos(), 10)); => float position[3];

        // for(0 => int i; i < instrumentArray.size(); i++){
        //     ((width / 2) )* -1 +
        // }
        
        // if(position[0] < instrumentArray.size[] )

        <<<"Sphere added at ", sphere.pos()>>>;
